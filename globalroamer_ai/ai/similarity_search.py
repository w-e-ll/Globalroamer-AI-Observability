...
from globalroamer_ai.core.exceptions import SimilaritySearchError
...
class SimilaritySearchService:
    ...
    def search_by_chunk(
        ...
        try:
            query_text = build_semantic_chunk_text(chunk)
...
def build_semantic_chunk_text(chunk: dict) -> str:
    """
    Build semantically rich text representation
    for embeddings and similarity search.
    """

    return f"""
    Telecom roaming operational trace.

    Testcase:
    {chunk.get("testcase_id")}

    Countries:
    {", ".join(chunk.get("countries", []))}

    Operators:
    {", ".join(chunk.get("operators", []))}

    Network Domains:
    {", ".join(chunk.get("network_domains", []))}

    Event Families:
    {", ".join(chunk.get("event_families", []))}

    Event Names:
    {", ".join(chunk.get("event_names", []))}

    Causes:
    {", ".join(chunk.get("causes", []))}

    Tags:
    {", ".join(chunk.get("tags", []))}

    Failure:
    {chunk.get("has_failure")}

    Retry Recommended:
    {chunk.get("has_retry_recommended")}
